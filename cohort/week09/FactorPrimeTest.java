import org.junit.Test;

import java.math.BigInteger;

public class FactorPrimeTest {
    @Test
    public void test1() {
        // Fermat number
        BigInteger n = new BigInteger("4294967297");
        BigInteger result = FactorPrime.factor(n);
        assert (result.intValue() == 641 || result.intValue() == 6700417);
    }

    @Test
    public void test2() {
        BigInteger n = new BigInteger("239839672845043");
        BigInteger result = FactorPrime.factor(n);
        assert (result.intValue() == 15486047 || result.intValue() == 15487469);
    }

    @Test
    public void test3() {
        BigInteger n = new BigInteger("1127451830576035879");
        BigInteger result = FactorPrime.factor(n);
        assert (result != null);
    }

    @Test
    public void testPerformance1() {
        BigInteger n = new BigInteger("4294967297");
        long startTime = System.currentTimeMillis();
        BigInteger one = FactorPrime.factor(n);
        long duration = System.currentTimeMillis() - startTime;
        int seconds = (int) (duration / 1000) % 60;
        int minutes = (int) ((duration / (1000 * 60)) % 60);
        int hours = (int) ((duration / (1000 * 60 * 60)) % 24);
        System.out.println(
                "Time used: " + hours + " hours " + minutes + " minutes " + seconds + " seconds.");
        assert (minutes <= 1);
    }

    @Test
    public void testPerformance2() {
        BigInteger n = new BigInteger("239839672845043");
        long startTime = System.currentTimeMillis();
        BigInteger one = FactorPrime.factor(n);
        long duration = System.currentTimeMillis() - startTime;
        int seconds = (int) (duration / 1000) % 60;
        int minutes = (int) ((duration / (1000 * 60)) % 60);
        int hours = (int) ((duration / (1000 * 60 * 60)) % 24);
        System.out.println(
                "Time used: " + hours + " hours " + minutes + " minutes " + seconds + " seconds.");
        assert (minutes <= 1);
    }

    @Test
    public void testPerformance3() {
        BigInteger n = new BigInteger("1127451830576035879");
        long startTime = System.currentTimeMillis();
        BigInteger one = FactorPrime.factor(n);
        long duration = System.currentTimeMillis() - startTime;
        int seconds = (int) (duration / 1000) % 60;
        int minutes = (int) ((duration / (1000 * 60)) % 60);
        int hours = (int) ((duration / (1000 * 60 * 60)) % 24);
        System.out.println(
                "Time used: " + hours + " hours " + minutes + " minutes " + seconds + " seconds.");
        assert (minutes <= 2);
    }

    @Test
    public void testPerformance4() {
        BigInteger n = new BigInteger(
                "");
        // How do we assert that it would take more than minutes to finish factoring this number? (We can use a timeout and assert whether this part of the process/thread is still alive by then.)
    }
}
